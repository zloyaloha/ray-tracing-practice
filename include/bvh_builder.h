...
__host__ inline AABB get_plane_aabb(const PlaneData& p) {
    ...
    if (p.type == QUAD || p.type == ELLIPSE) {
        min_x = fminf(min_x, p3.x());
        max_x = fmaxf(max_x, p3.x());
        min_y = fminf(min_y, p3.y());
        max_y = fmaxf(max_y, p3.y());
        min_z = fminf(min_z, p3.z());
        max_z = fmaxf(max_z, p3.z());
    }
...
__host__ int build_bvh_recursive(std::vector<BVHPrimitive>& primitives, int start, int end, std::vector<BVHNode>& nodes) {
    ...
    if (span == 1) {
        ...
        if (centroid_box.y_interval.size() > max_extent) {
            axis = 1;
            max_extent = centroid_box.y_interval.size();
        }
        if (centroid_box.z_interval.size() > max_extent) {
            axis = 2;
        }
