...
inline __host__ __device__ bool is_interior_quad(double a, double b, HitRecord &rec) {
...
inline __host__ __device__ bool is_interior_ellipse(double a, double b, HitRecord &rec) {
...
inline __host__ __device__ bool is_interior_triangle(double a, double b, HitRecord &rec) {
    if (a < 0 || b < 0 || (a + b) > 1) {
        return false;
    }
    rec.u = a;
...
inline __host__ __device__ bool hit_plane(const Ray &r, const Interval &ray_inter, HitRecord &rec, const PlaneData &plane_data) {
