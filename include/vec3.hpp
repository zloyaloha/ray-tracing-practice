...
inline double vec3::len() const { return std::sqrt(len_squared()); }
...
inline vec3 operator*(const vec3 &v2, double t) { return t * v2; }

inline vec3 operator/(const vec3 &v2, double t) { return (1.0 / t) * v2; }
...
inline vec3 vec3::unit_vector() const { return *this / this->len(); }
