...
inline __host__ __device__ bool hit_scene(const Ray &r, Interval ray_t, HitRecord &rec, const SceneData &scene_data_ref) {
    ...
    for (int k = 0; k < scene_data_ref.num_bvh_trees; ++k) {
        if (hit_bvh(r, Interval(ray_t.min, closest_so_far), temp_rec, scene_data_ref.d_bvh_trees[k].nodes,
                    scene_data_ref.d_bvh_trees[k].num_nodes, scene_data_ref.d_spheres, scene_data_ref.d_planes)) {
            hit_anything = true;
