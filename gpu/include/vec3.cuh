...
struct vec3 {
    ...
    __host__ __device__ inline vec3() = default;
    ...
    __host__ __device__ inline float operator[](std::size_t i) const { return e[i]; }
    __host__ __device__ inline float &operator[](std::size_t i) { return e[i]; }
    ...
    __host__ __device__ inline vec3 &operator*=(float alpha) {
        e[0] *= alpha;
        e[1] *= alpha;
        e[2] *= alpha;
        return *this;
    ...
    __host__ __device__ inline vec3 &operator/=(float alpha) { return *this *= (1.0 / alpha); }

    __host__ __device__ inline float len_squared() const { return e[0] * e[0] + e[1] * e[1] + e[2] * e[2]; }
    __host__ __device__ inline float len() const { return sqrtf(len_squared()); }
    ...
    __host__ __device__ inline vec3 reflect(const vec3 &n) const { return *this - 2 * dot(*this, n) * n; }

    __device__ vec3 inline refract(const vec3 &n, float etai_over_etat) const {
        float cos_theta = fminf(dot(-(*this), n), 1.0);
        vec3 r_out_perp = etai_over_etat * ((*this) + cos_theta * n);
        vec3 r_out_parallel = -sqrtf(fabsf(1.0 - r_out_perp.len_squared())) * n;
