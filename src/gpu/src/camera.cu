...
// Wrapper functions to copy data to constant memory (must be in same TU as definitions)
void set_camera_data_const(const CameraData& data) {
    cudaMemcpyToSymbol(d_cam_data_const, &data, sizeof(CameraData), 0, cudaMemcpyHostToDevice);
}

void set_scene_data_const(const SceneData& data) {
    cudaMemcpyToSymbol(d_scene_data_const, &data, sizeof(SceneData), 0, cudaMemcpyHostToDevice);
}
...
__host__ void render_kernel_cpu(color *framebuffer, const CameraData &d_cam_data_const, const SceneData &d_scene_data_const) {
    size_t total_pixels = d_cam_data_const.image_height * d_cam_data_const.image_width;
    size_t pixels_completed = 0;
    
    for (size_t j = 0; j < d_cam_data_const.image_height; ++j) {
        if (j % 10 == 0) {
            std::cerr << "\rProgress: " << (100 * pixels_completed / total_pixels) << "%" << std::flush;
        }
        for (size_t i = 0; i < d_cam_data_const.image_width; ++i) {
            color pixel_color(0, 0, 0);
            for (size_t sample = 0; sample < d_cam_data_const.samples_per_pixel; ++sample) {
                unsigned int local_rand_seed = wang_hash((i * d_cam_data_const.image_width) + j + sample);
                Ray r = d_cam_data_const.get_ray(i, j, local_rand_seed);
                pixel_color += ray_color_host(r, local_rand_seed, d_scene_data_const, d_cam_data_const);
            }
            framebuffer[j * d_cam_data_const.image_width + i] = pixel_color;
            pixels_completed++;
        }
    }
...
void Camera::render(SceneData *scene_data, color *d_fb) const {    
    ...
    set_camera_data_const(cam_data);
