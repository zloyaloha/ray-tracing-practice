...
__host__ __device__ inline float random_float(unsigned int &seed) {
    seed = wang_hash(seed);
...
__host__ __device__ inline vec3 random_unit_vector(unsigned int &seed) { return unit_vector(random_in_unit_sphere(seed)); }

__host__ __device__ inline vec3 random_in_hemisphere(const vec3 &normal, unsigned int &seed) {
