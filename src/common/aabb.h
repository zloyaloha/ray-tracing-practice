...
struct AABB {
    ...
    __host__ __device__ AABB(const point3& p1, const point3& p2) {
        x_interval = (p1.x() <= p2.x()) ? Interval(p1.x(), p2.x())
                                     : Interval(p2.x(), p1.x());
        y_interval = (p1.y() <= p2.y()) ? Interval(p1.y(), p2.y())
                                     : Interval(p2.y(), p1.y());
        z_interval = (p1.z() <= p2.z()) ? Interval(p1.z(), p2.z())
                                     : Interval(p2.z(), p1.z());
        expand_to_min();
    }
    ...
    __host__ __device__ bool hit(const Ray &r, Interval ray_t) const {
        for (int a = 0; a < 3; a++) {
            ...
            if (a == 0) { min_val = x_interval.min; max_val = x_interval.max; }
            else if (a == 1) { min_val = y_interval.min; max_val = y_interval.max; }
            else { min_val = z_interval.min; max_val = z_interval.max; }
...
__host__ __device__ inline AABB surround(const AABB &box0, const AABB &box1) {
