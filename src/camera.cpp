...
Camera::Camera(double ratio, int width, std::unique_ptr<ISaver> &image_saver)
    : aspectRatio(ratio), imageWidth(width), saver(std::move(image_saver)) {
    ...
    vec3 viewport_LR = vec3(viewportWidth, 0, 0),
         viewport_UD = vec3(0, -viewportHeight, 0);
    delta_pixel_LR = viewport_LR / imageWidth,
    delta_pixel_UD = viewport_UD / imageHeight;
    vec3 viewport_upper_left =
        coords - vec3(0, 0, focal_lenght) - viewport_LR / 2 - viewport_UD / 2;
