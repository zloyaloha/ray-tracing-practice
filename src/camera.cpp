...
FileSaver::FileSaver(int sampelsPerPixel, const std::string &filesaver)
    : ISaver(sampelsPerPixel), fout(filesaver) {}
...
void FileSaver::writeColor(color pixel_color) {
    ...
    fout << static_cast<int>(256 * intensity.clamp(r)) << ' '
...
PNGSaver::PNGSaver(int sampelsPerPixel, const std::string &filepath)
    : ISaver(sampelsPerPixel), filepath(filepath) {}
...
PNGSaver::~PNGSaver() {
    if (!pixel_data.empty()) {
        int result = stbi_write_png(filepath.c_str(), imageWidth, imageHeight,
                                    3, pixel_data.data(), imageWidth * 3);
...
Camera::Camera(double ratio, int width, std::unique_ptr<ISaver> &image_saver,
               const point3 &camera_pos, const point3 &look_at_point)
    : aspectRatio(ratio),
      imageWidth(width),
    ...
    vec3 viewport_upper_left = coords + focal_length * forward -
                               viewport_right / 2.0 + viewport_up / 2.0;
    zeroPixelLoc =
        viewport_upper_left + (delta_pixel_LR + delta_pixel_UD) * 0.5;
}

void Camera::render(const HittableObject &world) {
    int counter = 0;
    for (size_t j = 0; j < imageHeight; ++j) {
        std::clog << "\rProgress bar: " << (imageHeight - j) << ' '
                  << std::flush;
...
Ray Camera::getRay(int i, int j) const {
    point3 pixel_loc =
        zeroPixelLoc + (i * delta_pixel_LR) + (j * delta_pixel_UD);
